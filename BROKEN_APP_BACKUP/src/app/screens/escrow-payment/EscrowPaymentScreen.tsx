...
export default function EscrowPaymentScreen() {
  ...
  const loadDetails = useCallback(async () => {
    if (!jobId || !offerId) {
      Alert.alert(t('error'), 'Job ID and Offer ID are required', [{ text: t('ok') }]);
      router.back();
      return;
    }
    ...
    try {
      ...
      if (!jobDetails || !offerDetails) {
        Alert.alert(t('error'), 'Job or offer not found', [{ text: t('ok') }]);
        router.back();
        return;
      }
      ...
      Alert.alert(t('error'), 'Failed to load job and offer details', [{ text: t('ok') }]);
    } finally {
      setLoading(false);
    }
  }, [jobId, offerId, t]);
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      setProcessing(true);
      const clientId = 'demo-client-id'; // TODO: Get actual client ID from auth
      ...
      Alert.alert(t('error'), t('paymentFailed'), [{ text: t('ok') }]);
    } finally {
      setProcessing(false);
    }
