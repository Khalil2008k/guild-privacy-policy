...
export default function OfferSubmissionScreen() {
  ...
  const loadJobDetails = useCallback(async () => {
    if (!jobId) {
      Alert.alert(t('error'), 'Job ID is required', [{ text: t('ok') }]);
      router.back();
      return;
    }
    ...
    try {
      ...
      if (!jobDetails) {
        Alert.alert(t('error'), 'Job not found', [{ text: t('ok') }]);
        router.back();
        return;
      }
      ...
      Alert.alert(t('error'), t('failedToLoadJobs'), [{ text: t('ok') }]);
    } finally {
      setLoading(false);
    }
  }, [jobId, t]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      setSubmitting(true);
      const freelancerId = 'demo-freelancer-id'; // TODO: Get actual freelancer ID from auth
      ...
      Alert.alert(t('error'), t('offerSubmissionFailed'), [{ text: t('ok') }]);
    } finally {
      setSubmitting(false);
    }
