...
export function DebugStorageSection() {
  ...
  const handleAddToAsyncStorage = useCallback(async () => {
    if (!debugKey || !debugValue) {
      CustomAlertService.showError('Error', 'Please enter both key and value');
      return;
    }

    try {
      await AsyncStorage.setItem(debugKey, debugValue);
      CustomAlertService.showSuccess('Success', `Added item with key "${debugKey}" to AsyncStorage`);
      setDebugKey('');
      setDebugValue('');
    } catch (error) {
      CustomAlertService.showError('Error', `Failed to add item: ${String(error)}`);
    }
  }, [debugKey, debugValue]);
